...
var KTAppSidebar = (function () {
  ...
  var handleSecondaryWrapper = function () {
    ...
    if (
      KTUtil.isVisibleInContainer(menuActiveItem, secondaryWrapper) === true
    ) {
      return;
    }
  ...
  var handleTagsCollapse = function () {
    secondaryTagsCollapse.addEventListener("shown.bs.collapse", (event) => {
      if (
        KTUtil.isVisibleInContainer(secondaryTagsToggle, secondaryWrapper) ===
        true
      ) {
        return;
      }
      ...
      secondaryWrapper.scroll({
        top: KTUtil.getRelativeTopPosition(
          secondaryTagsToggle,
          secondaryWrapper
        ),
        behavior: "smooth",
  ...
  return {
    init: function () {
      // Elements
      secondaryWrapper = document.querySelector(
        "#kt_app_sidebar_secondary_wrapper"
      );
      secondaryTagsCollapse = document.querySelector(
        "#kt_app_sidebar_secondary_tags_collapse"
      );
      secondaryTagsToggle = document.querySelector(
