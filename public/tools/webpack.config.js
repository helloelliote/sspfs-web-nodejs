...
// get selected demo
let demo = getDemos(rootPath)[0] || "demo31";
...
function getEntryFiles() {
  ...
  if (isPresets) {
    if (fs.existsSync(srcPath + "/sass/presets")) {
      ...
      presets.forEach((preset) => {
        entries["plugins/global/plugins." + presets + ".bundle"] = [
          "./webpack/plugins/plugins.scss",
        ];
        entries["css/style." + presets + ".bundle"] = [
          "./" +
            path.relative("./", srcPath) +
            "/sass/presets/" +
            preset +
            "/style.scss",
          "./" +
  ...
  entries["js/widgets.bundle"] =
    glob.sync(path.relative("./", srcPath) + "/js/widgets/**/!(_)*.js") || [];
